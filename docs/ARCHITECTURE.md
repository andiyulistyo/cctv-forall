...
```mermaid
flowchart TB
    IN["Box hasil tracking<br/>class ∈ ALPR_CLASSES (car, truck, bus)<br/>+ ALPR_CAPTURE_CLASSES (car, truck, motorcycle, person) di zona<br/>kelas di dua daftar dapat dua-duanya: satu baris, plat menyusul<br/>person → face_sightings, sisanya → plate_reads"]
